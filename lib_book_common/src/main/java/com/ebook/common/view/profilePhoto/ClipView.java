...
public class ClipView extends View {
    ...
    protected void onDraw(@NonNull Canvas canvas) {
        ...
        if (clipType == ClipType.CIRCLE) {
            //中间的透明的圆
            canvas.drawCircle((float) this.getWidth() / 2, (float) this.getHeight() / 2, clipRadiusWidth, paint);
            //白色的圆边框
            canvas.drawCircle((float) this.getWidth() / 2, (float) this.getHeight() / 2, clipRadiusWidth, borderPaint);
        } else if (clipType == ClipType.RECTANGLE) { //绘制矩形裁剪框
            //绘制中间的矩形
            canvas.drawRect(mHorizontalPadding, (float) (this.getHeight() / 2 - clipWidth / 2),
                    this.getWidth() - mHorizontalPadding, (float) (this.getHeight() / 2 + clipWidth / 2), paint);
            //绘制白色的矩形边框
            canvas.drawRect(mHorizontalPadding, (float) (this.getHeight() / 2 - clipWidth / 2),
                    this.getWidth() - mHorizontalPadding, (float) (this.getHeight() / 2 + clipWidth / 2), borderPaint);
        }
