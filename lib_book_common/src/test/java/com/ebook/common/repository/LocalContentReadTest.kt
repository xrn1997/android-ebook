...
/**
 * [BookRepository.loadChapter] 的单元测试（本地书路径 + 定位值归属贯通）。
 *
 * 锁四件事：
 * 1. 本地书走 reader 不走数据库：返回的 `ChapterContent` 来自 reader
 * 2. 同一章翻多页只读盘一次（内存缓存兜住了第二次起）
 * 3. 未知格式回落到 TXT（resolveFormat 的 getOrDefault 行为）
 * 4. **交给 reader 的 [BookLocation] 带上这本书的 `tag`**（ADR-0016 P3-a）：网络书那条用例只断言
 *    reader 收到的定位值，真取源与抛异常由 `JsoupSourceReaderTest` 锁，这里不碰网络。
 */
class LocalContentReadTest {
     ...
     * [format] 决定这个 reader 注册在哪个格式上——网络书归属的断言要挂到 NETWORK 上才走得到。
     */
    private fun repositoryWith(
        ...
        bookGroupDao = daos.group,
        downloadChapterDao = daos.download,
        chapterReaders = mapOf(format to chapterReader),
        bookSourceManager = FakeBookSourceManager(),
    ...
    /**
     * 锁 ADR-0016 P3-a 的落点：仓库构造的 [BookLocation] 必须带上书架的 `tag`，
     * 否则网络正文读取就会退回「当前默认书源」去抓别的站。
     */
    @Test
    fun `网络书的定位值带上该书的书源归属`(): Unit = runTest {
        val networkReader = RecordingReader(ChapterContent("第一章", listOf("正文")))
        val repository = repositoryWith(networkReader, BookFormat.NETWORK)
        val shelf = BookShelfEntity(
            noteUrl = "https://site.example/book/1",
            tag = "https://bound.example",
        )
        ...
        repository.loadChapter(
            ...
            index = 0,
            title = "第一章",
            chapterContentRef = "https://site.example/ch/1.html",
        )

        assertEquals("https://bound.example", networkReader.lastLocation?.sourceUrl)
        assertEquals(BookFormat.NETWORK, networkReader.lastLocation?.format)
    }

    /** 本地书的「归属」就是 loc_book：reader 按格式路由到 TXT/EPUB，取值只用于表态一致 */
    @Test
    fun `本地书的定位值归属仍是 loc_book`(): Unit = runTest {
        val repository = repositoryWith(reader)
        val shelf = BookShelfEntity(
            noteUrl = "local-book-id",
            tag = BookShelfEntity.LOCAL_TAG,
            bookFormat = "TXT",
        ...
        repository.loadChapter(shelf, index = 0, title = "第一章")

        assertEquals(BookShelfEntity.LOCAL_TAG, reader.lastLocation?.sourceUrl)
    }

    /**
     * 记录调用的假 reader：返回固定 [ChapterContent]，统计 `readChapter` 被调用次数，
     * 并留下最后一次的定位值——归属是否随定位值一起下去，只有 reader 自己看得见。
     */
    private class RecordingReader(private val content: ChapterContent) : ChapterReader {
        var callCount = 0
            private set

        var lastLocation: BookLocation? = null
            private set
