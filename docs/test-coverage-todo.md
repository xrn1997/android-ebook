...
## 人工装机验证清单（阅读器上下滚屏翻页方式，2026-09-13）

块总数落定与哨兵解析、章间衔接、进度上报口径、失败块不塌陷由 `ReaderScrollControllerTest`（11 例）锁住；
加载去重与三态由 `ReaderPageStoreTest`（5 例）锁住；块高取实测行底部由 `ReaderBlockMetricsTest`
（4 例，纯函数）锁住；单选行的深浅色取色由 `ReaderTurnModeRowRenderTest` 锁住（探针已抽成 `ReaderRenderProbe`
与 `ReaderChromeThemeRenderTest` 共用）。**`ReaderPagerControllerTest` 一个用例都没改且原样通过**（7 例），
即本次抽出加载仓库对翻页行为零影响。`module_book` 全量 102 例、`:module_app:assembleRealDebug` 与
`assembleMockDebug` 均通过。以下只列自动化够不到的**设备项**。

1. **滚屏手感**：竖向自由滚动顺滑、fling 正常、**块与块之间既无缝隙也无叠行**。有缝说明块高不是排版实测值
   （`ReaderTypesetter.measureBlock`）；叠行说明块高大于实测值。这是本轮最该盯的一条。
2. **跨章连续**：往下滚过本章末块，紧接着出现的是下一章标题与正文（不再有「下一章」链接项要点）；
   往上滚过本章标题则接上一章末块。章界处若出现「列表到底了」的感觉，说明相邻章没被物化。
3. **接章无感**：进入某章即物化相邻两章，故未下载章节走网络时章界附近应已预热完；若章界处停在加载态，
   看 logcat 是相邻章正文还没抓回来（网络慢，可接受）还是压根没发请求（物化没触发，是缺陷）。
4. **模式切换落点**：左右翻页读到第 5 屏 → 设置面板切「上下滚屏」，落点应在同一段字附近；再切回来同样。
   偏了说明 `lastLineCount` 记晚了（必须在重分页之前）或换算用错了行数。
5. 字号/背景/亮度在滚屏模式下照常生效；改字号后重新分块且落点不丢（由 `LaunchedEffect(typesetter)` 接力）。
6. **点击与按键**：左右三分区点一下滚一屏（受「点击翻页」开关控制）、中间三分区唤菜单；音量上下键滚一屏
   （受「按键翻页」开关控制）。竖向拖拽必须交给 LazyColumn 的惯性与 fling，不得与点击判定打架。
7. **杀进程重进**应停在滚屏模式上次那一屏——偏移一屏即「`(章, 块)` ↔ 扁平 item 序号」的映射在控制器与
   容器两侧算得不一致；偏移若干屏则是章段展开时按 key 锚定失效（key 重复，或把扁平序号写进了 key）。
8. 深浅色切换：设置面板新增的「翻页方式」单选卡随 chrome 层走深浅色，正文块不受影响。
9. 两种方式的「没有上一页/没有下一页」提示分别在**整本书**首、书末出现（章首章末是接相邻章，不提示）。
10. 独立模块模式（`gradle.properties` 的 `isModule` 临时改 `true`，验完改回）下阅读器能正常起来
    ——两份 Manifest 口径未动，应无影响。

## 人工装机验证清单（内容仓库目录名派生 + 滚屏加载收敛，2026-09-13）

两处缺陷与修法见 ADR-0038 与 `ReaderPageStore` 类 KDoc。目录名派生与它的三处症状（对账误删、册数少算、
连带删除）由 `BookStoreTest`（16 例）锁住；往返滚动不重复请求已读块、块自请求兜住预取覆盖不到的块由
`ReaderScrollControllerTest`（11 例）锁住；保留集裁剪只有翻页一端做（滚屏一端已删，理由见 `ReaderPageStore`
类 KDoc），故 `ReaderPageStoreTest`（6 例）与 `ReaderPagerControllerTest`（7 例）原样通过。
以下只列自动化够不到的**设备项**。

1. **重启不丢缓存**（本轮主症状）：读一章网络书 → 杀进程重进 → 同一章应直接出正文，logcat 里不再有该章的
   正文抓取。也可在重启前后各跑一次
   `adb shell run-as <包名> find files/books -name 'c*.txt'`，**章文件数不得减少**；新目录名应是 32 位
   十六进制（`files/books/<md5>/cNNNNN.txt`），旧的 `files/books/https:/…` 在改后首次启动被对账回收。
2. **缓存管理页册数**：设置 → 缓存管理，「书籍内容」那行的册数应等于书架上的书目数（旧布局下所有网络书
   共用一个 `https:` 顶层目录，被数成 1 册）。
3. **回滚不闪加载态**：上下滚屏往下读 5～6 屏再滚回来，已读过的块应直接是正文、不出现转圈。
4. **fling 落定后出正文**：快速甩动滚到远处，落定那一屏应在片刻内出正文（块自请求兜底）；甩动过程中
   允许短暂加载态。若落定后长时间停在转圈，说明块的 `LaunchedEffect` 没接上 `ensureLoaded`。
5. **删书与「强制刷新缓存」仍失效正文缓存**：`cacheMarker` 改成按派生目录名匹配，这条是它的回归项——
   对某章强刷后，已打开的阅读器不得继续供给旧正文；删书后章文件目录应被清掉。

## 人工装机验证清单（书架网络书目录重抓阶段 1，2026-09-13）

判定口径与整笔放弃的取舍见 ADR-0039。前缀判定（含「本地有洞仍能追加」「缺掉的章不被补回来」
「相对顺序不一致判分叉」）由 `ChapterTocDiffTest`（14 例）锁住；序号不撞车、时间戳的写与不写、
失败不动库、事件只在追加时发由 `BookRepositoryChapterSyncTest`（18 例）锁住；
`final_refresh_data` 只动那一列由 `BookInfoDaoTest`（3 例）在真实 SQL 上锁住。
`lib_book_common` 全量单测与 `module_book`/`module_find` 编译均通过。以下只列自动化够不到的**设备项**。

1. **追加后不读错章**（本轮最该盯的一条）：挑一本已下载过若干章的网络书，等它出新区间后进详情页触发
   追加，然后翻回**追加之前**就已缓存的章节 —— 正文必须与追加前逐字一致。读到不同内容说明序号被重排
   错了，而它不报错也不闪退。
2. **真书进详情页看到「已更新 N 章」且目录变长**：末章应是新抓到的那一章，目录抽屉里能翻到它。
3. **断网进详情页**：不弹任何错误、不显示「加载失败」、本地目录照常可读、能正常进阅读器。
4. **读到末章能继续翻，且位置不变**：两种翻页方式（左右翻页 / 上下滚屏）都要试 —— 追加成功后
   停留的那一屏不得跳走，滑条与「共 M 章」的分母要跟着变。
5. **限频真的生效**：5 分钟内反复进出同一本书的详情页，抓包/网络日志应只出现一次目录请求。
6. **分叉提示条**：拿一本会把章节 URL 规则改掉的源（或临时改书源规则）制造分叉，详情页应出现
   「目录结构已变化，请换源或重新导入」，且这本书**仍可正常阅读**、本地目录一字未变。

## 人工装机验证清单（上下滚屏改跨章连续，2026-09-13）

模型与三条硬约束见 ADR-0037 决策 5 与 `ReaderScrollController` 类 KDoc。章段物化、扁平序号与 `(章, 块)`
双向映射、锚点存语义值故上方插入不动进度、哨兵落点解析与补报、标题项与首块之间来回不重复写进度、
保留集按章距裁剪由 `ReaderScrollControllerTest`（19 例）锁住；「本章末块 → 下一章标题 → 下一章首块」的
列表结构、章界链接项不再存在、容器重新进入组合时不把进度写到列表首项那一章、块高未知时占位不塌陷由
`ReaderScrollSeamlessTest`（3 例，Compose 渲染）锁住。`module_book` 全量 115 例、两个 flavor 的
`assembleDebug` 均通过（唯一的 lint error 仍是 `BookShelfPage.kt:117` 的存量问题，本轮文件零条目）。
以下只列自动化够不到的**设备项**。

1. **章段展开时画面不动**（本轮最该盯的一条，也是唯一依赖平台行为的一条）：在章界附近慢慢滚，相邻章由
   一个占位块展开成整章的那一刻，正在读的那一屏**不得有任何位移或闪跳**。位移说明 LazyColumn 的按 key
   锚定没生效——先查 item key 是否唯一、是否误把扁平序号写进了 key。
2. **fling 跨章不停顿**：从本章中段用力甩到下一章，动量应一路带过去，不在章界断掉、不需要「再点一下才继续」。
3. **往回滚出 ±2 章才重新加载**：连续往前读 5 章以上再一路滚回起点——锚点章 ±2 之外的正文已被裁掉，
   滚回去时会短暂转圈再出正文；±2 之内必须直接是正文、不转圈。
