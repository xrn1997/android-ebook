...
**实现补记（2026-09-07）**：最终落地为 `isMinifyEnabled = isModule`（独立态开启、集成态关闭），不再是「恒 true 不分支」——AGP 9 实测对 library 置 true 同样执行 R8，会提前剥离类、令 `module_app` 集成构建报 Missing class，故集成态必须关闭；独立态开启以尽早暴露规则缺口。最终口径以 ADR-0024 为准（§3.5 构建脚本清单与本补记一致）。
...
**实现补记（2026-09-07）**：最终实现未透传原始长度——`asResponseBody` 落地为单参调用，contentLength 固定为 -1（未知），动机是避免下游因已知长度触发全量缓冲（见实现 KDoc）。与「contentLength 原样保留」的设计在带 `Content-Length` 的响应上不等价，行为由 `EncodingInterceptorTest` 锁定（已知长度入参断言输出 -1）；该偏差已补记入 ADR-0024。
