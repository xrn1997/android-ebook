...
>
> **状态说明（2026-09-12 实施期修订，以代码与 ADR-0035/0034 修订说明为准）**：
> 1. D2 的单主 FAB 与列表尾部「取消全部」、D3 的全高 BottomSheet 承载，装机观感不符合预期，已被用户否决——一级页不再设集中控制按钮，「取消全部」删除；二级恢复整屏二级页（一级点书行进入、返回键回一级）。
> 2. 二级选章列表改**平铺整列 + 顶部快捷选择 + 右侧细滚动条**，「百章分组折叠」形态废弃（ADR-0034 修订说明）。
> 3. D1/D4/D5/D6 部分（轻书行、组件收编、启动信号化、bookSheet 装载态）保留，但状态机恢复为 `DownloadCenterStep`（Books/PickBook）导航 + `bookSheet` 装载结果的组合。
