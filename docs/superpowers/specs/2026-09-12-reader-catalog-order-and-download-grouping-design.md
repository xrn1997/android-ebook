...
- **软上限**（**只提醒、不阻止**）：`selected.size > MAX_DOWNLOAD_SELECTION(500)` 时，点确认先弹一次 `AlertDialog`——正文写明"本次将一次性下发 N 个下载任务。任务较多，下载耗时较长，期间会持续占用离线下载服务"，确认键用动作动词**「继续下载」**——确认后照常 `onConfirm`；≤ 500 零摩擦。500 恰为 5 组，与分组粒度同拍。**文案刻意不自称"上限"**：这个阈值可以被越过，用一条自己马上会推翻的规则做警示，会削弱后续所有警示的可信度；也刻意不写任何具体时长或配额数字——那由系统策略决定，写死会在策略变化时变成假话。
...
- **500 是产品阈值**：改一个常量即可调整；提醒弹窗的文案刻意不写死这个数字、也不自称"上限"（见 §4.4），面板上更无常驻的"N/500"反馈——可选增强，本次不做。
