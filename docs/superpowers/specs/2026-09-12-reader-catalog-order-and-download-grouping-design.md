...
2. **目录抽屉去掉 `key = { index, _ -> index }`**（`ReaderPanels.kt:839`）：index 作 key 与"不要 key"的位置身份完全等价，却让 LazyList 认为存在稳定 key 而常驻一张 N 项 `key→index` 表（装箱 Integer，数千章数百 KB，且 item provider 换实例时重建）。抽屉每行没有需要跨滚动保留的状态，直接去掉。**注意**：下载面板的新列表不受此影响，它必须带 key（组头与子项两类）。
