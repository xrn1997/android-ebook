...
class CommentRepository @Inject constructor(
    ...
    // 分页形态：翻页由调用方推进（首页 page = 1），pageSize 缺省取 PAGE_SIZE。
    // 空键列表在仓库层直接返回空页、不发请求——comment_keys 缺失时后端返回的是
    // 全局最新列表（见 §3.2.1），不收口就会把全站最新评论当成该章节的评论。
    suspend fun getComments(
        commentKeys: List<String>,
        page: Int,
...
// 并集查询（首页；翻页由调用方推进，取 page + 1）
val page1 = commentRepository.getComments(chapterKeys, page = 1)
```
