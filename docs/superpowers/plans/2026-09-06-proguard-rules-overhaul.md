...
> **状态说明（2026-09-10 追加）**：本计划**已执行完毕**（release R8 已开、`EncodingInterceptor` 的私有字段反射已去掉，
> 其残留的 `getDeclaredField` 字样只出现在解释历史故障的注释里）。勾选框全是 `- [ ]` 是因为本文是一次性施工记录、
> 跑完未回写，**不代表待办**。
> 另有两处**落地形态与上面 Architecture 段的设想不同**，照本文动工前先看清：
>
> - 不是「一份 `proguard-rules.pro` 双挂载」，而是**每模块两份文件**：`consumer-rules.pro` 挂
>   `consumerProguardFiles`（规则内容的唯一来源），`proguard-rules.pro` 挂 `buildTypes.release.proguardFiles`
>   且内容只有 `-include consumer-rules.pro`。功能模块的 `proguardFiles` 也不挂 app 那份。
> - 「lib 模块零手写规则」已不再成立：`lib_book_source/consumer-rules.pro` 现有一条
>   `-keep class com.ebook.source.sandbox.HostDispatcher { *; }`——JS 沙箱的 JNI 侧用 `FindClass`
>   按字符串名找 `handle`，是后来那批工作引入的新反射面。**这是当时的合理演进，不是本计划漏做的项。**
>
> 最终口径见 ADR-0024 与 AGENTS.md 的「混淆规则约定」；实况以代码、KDoc 与 ADR 为准。
...
计划正文与 checkbox 是执行时的历史快照，以下两处与最终实现不符，阅读时以本补记为准（出处：ADR-0024 与 spec 对应小节的实现补记）：

- 功能模块 minify 开关（Task 5-10 正文）：写的是 `isMinifyEnabled = true`（不按 isModule 分支）；最终落地为 `isMinifyEnabled = isModule`——集成态（library）必须关闭，否则 AGP 9 会对 library 执行 R8 剥离类、令 `module_app` 集成构建报 Missing class，独立态开启以尽早暴露规则缺口。
- EncodingInterceptor contentLength（Task 1 测试与实现）：写的是「原样保留」的双参 `asResponseBody(contentType, body.contentLength())`；最终落地为单参 `asResponseBody(contentType)`，contentLength 固定 -1（未知），避免下游因已知长度触发全量缓冲。
