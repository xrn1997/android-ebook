...
> **状态说明（2026-09-10 追加）**：本计划**已执行完毕**，产出在 HEAD 内（`ChapterReader`、`JsoupSourceReader`、
> 复用 `BookStore` 的读写链路、`DownloadRepository`/`DownloadService` 改造等交付文件**已逐个核对存在**）。
> 勾选框全是 `- [ ]` 只因本文是一次性施工记录、跑完未回写，**不代表待办**。
> 特别提示：文中点名的 `BookContentDao.kt` / `BookContentEntity.kt` **在现仓不存在，而这正是本计划的验收结果**——
> 正文脱离 `book_content` 表、该表由 v3→v4 迁移删除。不要据此判断"工作没做完"，更不要顺手把那张表请回来。
> 实况以代码、KDoc 与 ADR 为准。
