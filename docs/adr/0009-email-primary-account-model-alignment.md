...
服务端将账号主标识从**用户名**改为**邮箱**（即「邮箱即主键的注册与账号模型」，落地锚点见文末提交
`b322b96`），客户端随之对齐：登录用「邮箱 + 密码」；注册改为三步（邮箱发码 → 验证码 + 密码
...
- 「只有 login 发 token」收束了凭证签发语义：注册成功 ≠ 已建立用户会话，会话只由登录（及刷新）
  产生，边界清晰。
...
- **继续用户名主标识**：与服务端账号契约冲突（邮箱为登录主标识），且「注册必填用户名」是多余摩擦。
...
- `module_me`：编辑资料页「修改密码」入口从 `MODIFY_PATH`（忘记密码）改指
  `MODIFY_PWD_PATH`（已登录改密）——已持有会话时邮箱验证码身份核验形同虚设，且会绕过旧密码校验。
