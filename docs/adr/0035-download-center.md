...
1. **选章与管理合一为下载中心的两级结构**：一级按书任务列表（状态 + 队列剩余 + 正在下载第 N 章 + 全书覆盖率 + 取消），点书卡进二级；二级是该书全部章节的分组勾选列表，每行叠加下载状态（已缓存 / 待下载 / 下载中）。阅读器「下载」入口直达该书二级。选章以**全高 BottomSheet** 承载于下载管理页（而非独立整屏二级页），既不打断一级列表现场，又保留整屏可扫性（底部面板半屏扫描的历史问题见动机）。
