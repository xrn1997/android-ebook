# 客户端评论与用户资料接口对齐后端 ADR-0011（RESTful 契约）

客户端评论/用户资料接口整体对齐 ebook-server 的 ADR-0011 契约：评论接口从历史路径
...
- 后端 ADR-0011 已定稿新契约（章节冗余快照 + 分页包裹 + 上传端点），客户端必须对齐才能联通。
...
- **字段命名**：`Comment` 的 `comment` → `content`；`chapter_url/chapter_name/book_name/add_time`
  走 `@SerialName` 边界翻译；`user` 复用 `User` 实体解析（`uid/avatar` 已对齐，`email/password`
  默认值兜底，服务端不返回）。
...
- **User 实体复用而非新建 CommentUserView**：后端评论内嵌用户仅四字段，客户端复用 `User`
  少一个 DTO 和一组 mapper；代价是 `password/email` 字段走默认值（客户端 `User.password`
  仅本地构造请求用，无泄漏面）。
- **章节查询不带 book_name**：后端按 `chapter_url` 聚合即可，`book_name` 冗余参数暂不传，
  减少客户端状态传递。
- **后端 ADR-0011 刻意不校验 chapter_url 格式**（无书源数据），客户端对第三方书源 URL
  原样透传，两端一致。
