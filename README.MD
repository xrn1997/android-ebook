...
目前，APP已经升级到 **JDK 17**，最低支持 Android 8.0（API 26），并且能够在 Android 15 上运行。
...
- 完成 Java → Kotlin 100% 迁移
- 完成 Dagger → Hilt 迁移（全项目使用 `@HiltViewModel` 和 `@Inject`）
...
- **⏳ 正在进行**: suspend + Coroutines 替换 RxJava3（新代码使用 Coroutines，旧代码逐步迁移）
