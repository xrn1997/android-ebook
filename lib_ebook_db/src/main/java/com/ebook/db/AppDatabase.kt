 ...
 * 作品分组（book_group）。其中 `book_group` 承载作品身份（评论桶键与来源条目的关联），
 * 详见 `BookGroupEntity` KDoc。本类只声明表与 DAO 的对应关系，读写一律经各 DAO 由其
 * 上层仓库（`lib_book_common` 的 BookRepository、`module_book` 的 DownloadRepository 等）
 * 发起。
 *
 ...
 * （原名 `dur_chapter_url`，改名以承载本地章文件路径），同一 URL 天然只存一份、
 * upsert 语义清晰；下载任务与搜索历史是流水型数据，用自增 `id`，去重责任上移到仓库
 * （如 DownloadRepository.addTasks 按 durChapterUrl 查重）。
 *
