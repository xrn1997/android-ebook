...
public class BookShelfAdapter extends RefreshRecyclerViewAdapter {
    ...
    public interface OnItemClickListener {
        ...
        void onClick(BookShelf bookShelf, int index);

        void onLongClick(View view, BookShelf bookShelf, int index);
    }
    ...
    private void bindOtherViewHolder(final OtherViewHolder holder, int index) {
        ...
        Glide.with(holder.ivCover_1.getContext()).load(books.get(index_1).getBookInfo().getCoverUrl()).dontAnimate().diskCacheStrategy(DiskCacheStrategy.AUTOMATIC).centerCrop().placeholder(R.drawable.img_cover_default).into(holder.ivCover_1);
        holder.tvName_1.setText(books.get(index_1).getBookInfo().getName());
        ...
        if (index_2 < books.size()) {
            ...
            Glide.with(holder.ivCover_2.getContext()).load(books.get(index_2).getBookInfo().getCoverUrl()).dontAnimate().diskCacheStrategy(DiskCacheStrategy.AUTOMATIC).centerCrop().placeholder(R.drawable.img_cover_default).into(holder.ivCover_2);
            holder.tvName_2.setText(books.get(index_2).getBookInfo().getName());
            ...
            if (index_3 < books.size()) {
                ...
                Glide.with(holder.ivCover_3.getContext()).load(books.get(index_3).getBookInfo().getCoverUrl()).dontAnimate().diskCacheStrategy(DiskCacheStrategy.AUTOMATIC).centerCrop().placeholder(R.drawable.img_cover_default).into(holder.ivCover_3);
                holder.tvName_3.setText(books.get(index_3).getBookInfo().getName());
    ...
    private void bindLastestViewHolder(final LastestViewHolder holder, final int index) {
        if (books.size() == 0) {
            ...
            Glide.with(holder.ivCover.getContext()).load(books.get(index).getBookInfo().getCoverUrl()).dontAnimate().diskCacheStrategy(DiskCacheStrategy.AUTOMATIC).centerCrop().placeholder(R.drawable.img_cover_default).into(holder.ivCover);

            holder.flLastestTip.setVisibility(View.VISIBLE);

            holder.tvName.setText(String.format(holder.tvName.getContext().getString(R.string.tv_book_name), books.get(index).getBookInfo().getName()));

            if (null != books.get(index).getBookInfo() && null != books.get(index).getBookInfo().getChapterlist() && books.get(index).getBookInfo().getChapterlist().size() > books.get(index).getDurChapter()) {
                holder.tvDurprogress.setText(String.format(holder.tvDurprogress.getContext().getString(R.string.tv_read_durprogress), books.get(index).getBookInfo().getChapterlist().get(books.get(index).getDurChapter()).getDurChapterName()));
            }
            holder.llDurcursor.setVisibility(View.VISIBLE);
            holder.mpbDurprogress.setVisibility(View.VISIBLE);
            holder.mpbDurprogress.setMaxProgress(books.get(index).getBookInfo().getChapterlist().size());
            float speed = books.get(index).getBookInfo().getChapterlist().size()*1.0f/100;
    ...
    public synchronized void replaceAll(List<BookShelf> newDatas) {
        books.clear();
    ...
    private void order() {
        if (books != null && books.size() > 0) {
            for (int i = 0; i < books.size(); i++) {
                ...
                BookShelf tempBookShelf = books.get(i);
                books.set(i, books.get(temp));
                books.set(temp, tempBookShelf);
            }
        }
    }
