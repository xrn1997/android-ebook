 ...
 *
 * 与书架导入链路的判重差异（刻意保持）：本入口只有 [LocalBookImporter] 内置的 md5 精确
 * 判重（同一份文件重开短路），不做 comment_key 模糊判重处置——外部打开的主诉求是「立刻
 * 能读」，而模糊判重自带误报（同名无作者误并、作者解析不一致漏判），宁可漏提示也不弹
 * 处置框挡路；要完整处置语义请走书架导入页。
 *
