 ...
 *
 * 双用途：创建评论请求体（[content] 必填、[commentKey] 必填，章节字段可选）+ 评论响应项。
 * 服务端载荷为蛇形命名，Kotlin 属性保持驼峰，边界翻译由 [SerialName] 完成：
 * - [commentKey] ↔ comment_key（评论聚合键，M2 新增；创建评论必填）
 * - [chapterUrl]（已废弃，保留兼容）/[chapterName]/[bookName] ↔ chapter_url/chapter_name/book_name
